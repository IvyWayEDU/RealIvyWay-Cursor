...
import { PRICING_CATALOG, PricingKey } from '../lib/pricing/catalog';
import { toStripePriceEnvKey } from '../lib/pricing/stripePriceIds';
...
function pickTaxCodeIdByKeywords(codes: Stripe.TaxCode[], keywords: string[]): string | undefined {
...
async function resolveTaxCodes(): Promise<TaxCodeMap> {
  const fromEnv: TaxCodeMap = {
    education: process.env.STRIPE_TAX_CODE_EDUCATION || undefined,
    digital: process.env.STRIPE_TAX_CODE_DIGITAL || undefined,
  };
  if (fromEnv.education && fromEnv.digital) return fromEnv;
...
function productNameForKey(key: PricingKey): string {
  switch (key) {
...
async function ensureProduct(key: PricingKey, taxCodes: TaxCodeMap): Promise<string> {
...
async function ensurePrice(key: PricingKey, productId: string): Promise<string> {
  ...
  const match = list.data.find((p) => {
    ...
    if (p.tax_behavior !== 'exclusive') return false;
    if (isSubscription) {
...
async function main() {
  ...
  for (const key of keys) {
    ...
    const envKey = toStripePriceEnvKey(key);
    out[envKey] = priceId;
