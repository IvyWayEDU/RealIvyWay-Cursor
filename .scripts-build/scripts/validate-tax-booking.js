...
async function main() {
    ...
    const expectedPayout = Math.floor((0, getProviderPayout_1.getProviderPayout)('college_counseling') * 100);
    const problems = [];
    if (base !== expectedBase)
        problems.push(`Expected session_price_cents=${expectedBase}, got ${base}`);
    if (payout !== expectedPayout)
        problems.push(`Expected provider_payout_cents=${expectedPayout}, got ${payout}`);
    if (tax == null)
        problems.push('Missing tax_amount_cents (did Stripe Tax run?)');
    if (tax != null && tax < 0)
        problems.push(`Invalid tax_amount_cents=${tax}`);
    if (total == null)
        problems.push('Missing total_charge_cents / amountChargedCents');
