...
function isNoShowMarked(s) {
    ...
    if (noShowParty === 'provider' || noShowParty === 'both')
        return true;
    const attendanceFlag = String(s?.attendanceFlag || '').trim().toLowerCase();
    if (attendanceFlag === 'provider_no_show' || attendanceFlag === 'full_no_show')
        return true;
    if (s?.flagNoShowProvider === true)
        return true;
    return false;
}
async function main() {
