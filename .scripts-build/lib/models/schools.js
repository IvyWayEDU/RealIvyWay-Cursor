...
exports.SCHOOLS_LIST = void 0;
exports.getSchoolById = getSchoolById;
exports.findSchoolByDisplayName = findSchoolByDisplayName;
exports.searchSchools = searchSchools;
exports.mapLegacySchoolNameToId = mapLegacySchoolNameToId;
exports.normalizeSchoolId = normalizeSchoolId;
/**
...
function getSchoolById(id) {
    return exports.SCHOOLS_LIST.find(school => school.id === id);
}
/**
 * Find school by display name (case-insensitive, partial match)
 */
function findSchoolByDisplayName(name) {
    const normalized = name.trim().toLowerCase();
    return exports.SCHOOLS_LIST.find(school => school.displayName.toLowerCase() === normalized ||
...
function searchSchools(query) {
    if (!query.trim())
        return exports.SCHOOLS_LIST;
    const normalized = query.trim().toLowerCase();
    return exports.SCHOOLS_LIST.filter(school => school.displayName.toLowerCase().includes(normalized) ||
 ...
 */
function mapLegacySchoolNameToId(legacyName) {
    if (!legacyName || !legacyName.trim())
        return null;
    const normalized = legacyName.trim().toLowerCase();
    // Try exact match first
    const exactMatch = exports.SCHOOLS_LIST.find(school => school.displayName.toLowerCase() === normalized);
    if (exactMatch)
        return exactMatch.id;
    // Try partial match (legacy name contains school name or vice versa)
    const partialMatch = exports.SCHOOLS_LIST.find(school => {
    ...
    const variationKey = Object.keys(variations).find(key => normalized.includes(key) || key.includes(normalized));
    if (variationKey) {
