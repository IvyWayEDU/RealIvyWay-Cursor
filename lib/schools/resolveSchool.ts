...
export async function resolveOrCreateSchoolByName(input: string): Promise<ResolvedSchool> {
  ...
  if (existing?.id) {
    return {
      ...
      name: typeof existing.name === 'string' && existing.name.trim() ? existing.name.trim() : name,
      normalized,
    };
  }
  ...
  return {
    ...
    name:
      typeof (inserted as any)?.name === 'string' && (inserted as any).name.trim()
        ? (inserted as any).name.trim()
        : name,
    normalized,
  };
}
