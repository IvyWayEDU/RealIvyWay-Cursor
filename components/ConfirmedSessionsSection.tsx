...
export default function ConfirmedSessionsSection() {
  ...
  useEffect(() => {
    const fetchSessions = async () => {
      try {
        const res = await fetch('/api/sessions/upcoming?role=student', { method: 'GET' });
        if (!res.ok) {
        ...
        const upcoming = all.sort((a, b) => getStartMs(a) - getStartMs(b));
                  ...
                  <div className="flex sm:justify-end">
                    <div className="flex flex-col sm:flex-row gap-2 sm:items-center sm:justify-end w-full sm:w-auto">
                      <button
                        type="button"
                        onClick={() => {
                          const otherUserId = typeof (session as any)?.providerId === 'string' ? (session as any).providerId : '';
                          if (!otherUserId) return;
                          router.push(`/dashboard/messages?userId=${encodeURIComponent(otherUserId)}`);
                        }}
                        disabled={!session.providerId}
