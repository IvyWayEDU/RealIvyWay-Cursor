...
interface OnboardingStep2ProviderTypeProps {
  isTutor: boolean;
  isCounselor: boolean;
  onUpdate: (updates: { isTutor: boolean; isCounselor: boolean }) => void;
}

export default function OnboardingStep2ProviderType({
  isTutor,
  isCounselor,
  onUpdate,
}: OnboardingStep2ProviderTypeProps) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-900">Provider Type</h2>
        <p className="mt-2 text-sm text-gray-600">
          How would you like to help students? You can select both options.
        </p>
      </div>
      ...
      {!isTutor && !isCounselor && (
        <p className="text-sm text-red-600 mt-2">
          Please select at least one provider type to continue.
        </p>
      )}
    </div>
  );
